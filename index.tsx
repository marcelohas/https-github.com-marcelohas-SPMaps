...
// Extend Window interface to include Popup
declare global {
  interface Window {
    Popup: any;
  }
...
let directionsService; // Service for fetching real routes
...
async function initMap() {
  ...
  map = new Map(document.getElementById('map'), {
    center: {lat: -23.5505, lng: -46.6333}, // Default center: São Paulo
...
1. **Geographic Knowledge:** You possess extensive knowledge of:
   ...
   * Specific expertise in urban planning and transit (like SPMaps)
...
* When suggesting routes, prefer "TRANSIT" or "WALKING" for dense urban areas like São Paulo if suitable.
...
if (plannerModeToggle) {
  plannerModeToggle.addEventListener('change', () => {
    ...
    promptInput.placeholder = isPlannerMode
      ? "Create a day plan in... (e.g. 'One day in São Paulo' or 'Explore Paulista Avenue')"
...
// Helper to determine travel mode from string
function getTravelMode(transport) {
  const t = (transport || '').toLowerCase();
  if (t.includes('walk') || t.includes('foot')) return google.maps.TravelMode.WALKING;
  if (t.includes('bike') || t.includes('cycle')) return google.maps.TravelMode.BICYCLING;
  if (t.includes('bus') || t.includes('train') || t.includes('transit') || t.includes('subway') || t.includes('metro')) return google.maps.TravelMode.TRANSIT;
  return google.maps.TravelMode.DRIVING;
}

// Adds a line (route) between two locations on the map.
// Uses Google Maps Directions API for real routing when possible.
async function setLeg(args) {
  ...
  // Attempt to get real directions
  const travelMode = getTravelMode(args.transport);
  ...
  try {
    ...
    if (response.status === 'OK' && response.routes.length > 0) {
      // Use the detailed path from directions
      const routePath = response.routes[0].overview_path;
      
      const realPolyOptions = {
        strokeColor: isPlannerMode ? '#2196F3' : '#CC0099',
        strokeOpacity: 0.8,
        strokeWeight: isPlannerMode ? 5 : 4,
        map,
        path: routePath
      };

      const poly = new google.maps.Polyline(realPolyOptions);
      
      lines.push({
        poly,
        name: args.name,
        transport: args.transport,
        travelTime: response.routes[0].legs[0]?.duration?.text || args.travelTime,
      });
    ...
    // Fallback to straight/geodesic line if directions fail (e.g. across oceans)
    console.warn("Could not fetch directions, falling back to geodesic line:", error);
